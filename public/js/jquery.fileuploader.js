...
captions: {
button: function(options) { return 'Выбрать ' + (options.limit == 1 ? '' : ''); },
feedback: function(options) { return 'Выберите ' + (options.limit == 1 ? 'фото' : 'фото') + ' для загрузки'; },
feedback2: function(options) { return options.length + ' ' + (options.length > 1 ? '' : '') + ' '; },
drop: 'Drop the files here to Upload',
paste: '<div class="fileuploader-pending-loader"><div class="left-half" style="animation-duration: ${ms}s"></div><div class="spinner" style="animation-duration: ${ms}s"></div><div class="right-half" style="animation-duration: ${ms}s"></div></div> Pasting a file, click here to cancel.',
removeConfirmation: 'Вы действительно хотите удалить документ?',
errors: {
filesLimit: 'Максимальное количество загружаемого документа - ${limit}',
filesType: ' Документ в неподдерживаемом формате. Допустимые форматы - ${extensions}',
fileSize: ' Размер документа (${name} - ${fileMaxSize}MB) превышает максимально допустимый(5Mb)',
filesSizeAll: 'Размер документа (${name} - ${fileMaxSize}MB) превышает максимально допустимый(5Mb)',
fileName: 'Файл с именем ${name} уже загружен.',
folderUpload: 'Файл не выбран.'
}
}
}
})(jQuery);